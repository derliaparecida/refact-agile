...
class InstanceHandlerES(BrainiakRequestHandler):
    ...
    async def get(self, context_name, class_name, instance_id):
        ...
        with request_duration.labels(
            ...
            # if context_name == "glb":
            #     self.query_params["class_uri"] = f"http://semantica.globo.com/{class_name}"
            #     self.query_params["instance_uri"] = f"http://semantica.globo.com/{class_name}/{instance_id}"
    ...
    async def patch(self, context_name, class_name, instance_id):
        ...
        if settings.USE_NEPTUNE:
            ...
            # Retrieve original data
            instance_data_neptune = await get_instance(self.query_params)

            if instance_data_neptune is not None:
                # Editing an instance
                instance_data_neptune = instance_data_neptune["body"]
                instance_data_neptune.pop(
                    "http://www.w3.org/1999/02/22-rdf-syntax-ns#type", None
                )

                # compute patch
                changed_data = apply_patch(instance_data_neptune, patch_list)

                # Try to put
                await edit_instance_neptune(self.query_params, changed_data)
                status = 200
                ...
                instance_data_neptune = await get_instance_from_db(self.query_params)
                ...
                instance_uri, instance_id = await create_instance_neptune(
        ...
        if instance_data is not None:
            ...
            if instance and settings.NOTIFY_BS_EVENTS:
                ...
                if instance_data and settings.NOTIFY_BS_EVENTS:
                    self.query_params["instance_uri"] = instance_data2["@id"]
                    await self._notify_bs_events(
                        action="PUT", instance_data=instance_data2
                    )
    ...
    async def put(self, context_name, class_name, instance_id):
        ...
        with request_duration.labels(
            ...
            if settings.USE_NEPTUNE:
                try:
                    if not await instance_exists_neptune(self.query_params):
                        try:
                            schema = await schema_resource.get_cached_schema(
                                self.query_params
                            )
                        ...
                        instance_uri_neptune, instance_id_neptune = (
                            await create_instance_neptune(
                                self.query_params,
                                instance_data,
                                self.query_params["instance_uri"],
                            )
                        ...
                        status = 201
                        self.set_header("location", resource_url)
                        self.set_header("X-Brainiak-Resource-URI", instance_uri_neptune)

                    else:
                        await edit_instance_neptune(self.query_params, instance_data)
                        status = 200
